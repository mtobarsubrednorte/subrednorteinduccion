        ...
        <div x-cloak x-show="moduloSeleccionado == {{ $modulo->id }}">
            ...
            <div class="clase-info">
                <div class="clase-meta">
                    <div class="meta-item"><i class="far fa-clock"></i> Duración: {{ $modulo->duration ?? 'N/A' }} min
                    </div>
                </div>
            ...
            @if ($modulo->genilay_recursos_link1)
                ...
                <div class="genially-container">
                    <iframe title=".." frameborder="0" width="1200px" height="675px"
                        ...
                        src="{{ $modulo->genilay_recursos_link1 }}" type="text/html" allowscriptaccess="always"
        ...
        @foreach ($modulo->submodulos ?? [] as $sub)
            <div x-cloak x-show="moduloSeleccionado == {{ $sub->id }}">
                ...
                <div class="clase-info">
                    <h3>{{ $sub->title }}</h3>
                    <div class="clase-meta">
                        <div class="meta-item"><i class="far fa-clock"></i> Duración: {{ $sub->duration ?? 'N/A' }} min
                        </div>
                    </div>
                    <div class="clase-descripcion">
                        <p>{{ $sub->description }}</p>
                ...
                <div class="recursos">
                    <ul>
                        @foreach ($sub->recursos ?? [] as $recurso)
                            <li><a href="{{ Storage::url($recurso->file_path) }}"
                                    target="_blank">{{ basename($recurso->file_path) }}</a>
                            </li>
                        @endforeach
                    </ul>
            ...
            <div class="modal" id="modal-{{ $step->id }}">
                <div class="modal-content">
                    ...
                    @if (!empty($step->tips))
                        ...
                        @if (is_array($tips))
                            <div class="tips-section-collapsed" id="tipsBox">

                                <h3 class="tips-title">Consejos:</h3>

                                @foreach ($tips as $tipBlock)
                                    <h4 class="tips-subtitle">{{ $tipBlock['title'] ?? 'Sin título' }}</h4>

                                    @if (isset($tipBlock['items']) && is_array($tipBlock['items']))
                                        <ul class="tips-list">
                                            @foreach ($tipBlock['items'] as $item)
                                                <li>{{ $item }}</li>
                                            @endforeach
                                        </ul>
                                    @endif
                    ...
                    <div class="modal-footer">
                        ...
                        <button id="btn-complete-{{ $step->id }}" class="btn btn-primary"
        ...
        <div class="clase-info">
            ...
            @else
                <div class="certificate-preview">
                    ...
                    <a href="{{ route('certificate.download') }}" id="btnCertificado"
                        class="btn btn-primary mt-3 btn-certifycate" onclick="bloquearBoton(this)">
...
<script>
    ...
    document.getElementById('exam-form').addEventListener('submit', function(e) {
        ...
        if (answeredQuestions < totalQuestions) {
            if (!confirm(
                    `Has respondido ${answeredQuestions} de ${totalQuestions} preguntas. ¿Estás seguro de que quieres enviar el examen?`
                )) {
                e.preventDefault();
            }
        }
