...
function renderStep(data = {}) {
    ...
    div.innerHTML = `
        ...
        <div class="mb-3">
            <label class="small fw-bold text-muted">Selecciona quien puede acceder a este paso</label>
            <select 
                name="steps[${sIdx}][perfil_id][]" 
                class="form-select mb-3" 
                id="perfiles-select-${sIdx}" 
                multiple
            >
            </select>
        </div>
        ...
        <div class="row g-3">
            ...
            if (imageExt.includes(ext)) {
                return `<div class="mb-2"><img src="/storage/${data.file}" class="img-fluid rounded" style="max-height:150px"></div>`;
            }
            if (videoExt.includes(ext)) {
                return `<div class="mb-2"><video src="/storage/${data.file}" controls style="max-height:150px; width:100%"></video></div>`;
            }
            return '';
        })()}
    ...
    const perfilesDisponibles = window.existingData?.perfiles || window.existingData?.periles;

    if (perfilesDisponibles) {
        const perfilesSelect = div.querySelector(`#perfiles-select-${sIdx}`);

        // 1. Intentamos convertir el string '["1","2"]' a un Array real
        let selectedIds = [];
        try {
            if (typeof data.perfiles_id === 'string') {
                selectedIds = JSON.parse(data.perfiles_id);
            } else if (Array.isArray(data.perfiles_id)) {
                selectedIds = data.perfiles_id;
            }
        } catch (e) {
            console.error("Error al parsear perfiles_id:", e);
            selectedIds = [];
        }

        // 2. Normalizamos a String para comparar con seguridad
        const normalizedSelected = selectedIds.map(String);

        perfilesDisponibles.forEach(perfil => {
            const option = document.createElement('option');
            option.value = perfil.id;
            option.text = perfil.name;

            // 3. Comprobamos si el ID del perfil actual está en los seleccionados
            if (normalizedSelected.includes(String(perfil.id))) {
...
function renderImagen(data = {}) {
    ...
    div.innerHTML = `    
        <img src="/storage/${data.image_path}" alt="Imagen del Módulo" class="img-fluid mb-3">

        ${data.id ? `
            
            <input type="hidden"
                name="delete_imagenes[]"
                id="delete-imagen-${data.id}"
                value="${data.id}"
                disabled>
        ` : ''}
        ...
        <div class="d-flex justify-content-between align-items-center mb-2">
            ...
            <button type="button"
                class="btn btn-sm btn-outline-danger border-0 remove-parent-img"
        ...
        <input type="text"
            name="imagenes[${imagenIndex}][description]"
...
function renderPregunta(data = {}) {
    ...
    div.innerHTML = `
        ...
        <div class="d-flex justify-content-between mb-2">
            <label class="fw-bold text-primary"><i class="fas fa-question-circle me-1"></i> Pregunta #${pIdx + 1}</label>
            <button type="button" class="btn btn-sm btn-outline-danger border-0 remove-parent">Eliminar</button>
        </div>
        
        <div class="mb-3">
            <label class="small fw-bold text-muted">Selecciona quién puede ver esta pregunta</label>
            <select 
                name="preguntas[${pIdx}][perfiles_id][]" 
                class="form-select" 
                id="perfiles-select-preg-${pIdx}" 
                multiple
            >
            </select>
        </div>
    ...
    container.appendChild(div);
    console.log('Data de pregunta:', data.perfiles_id);
    // --- LÓGICA DE PERFILES PARA PREGUNTAS ---
    const perfilesDisponibles = window.existingData?.perfiles || window.existingData?.periles;

    if (perfilesDisponibles) {
        const perfilesSelect = div.querySelector(`#perfiles-select-preg-${pIdx}`);

        // Normalizar los IDs seleccionados que vienen de la base de datos
        let selectedIds = [];
        try {
            if (typeof data.perfiles_id === 'string') {
                selectedIds = JSON.parse(data.perfiles_id);
    ...
    if (data.opciones && Array.isArray(data.opciones)) {
        data.opciones.forEach((opc, oIdx) => {
            // Verifica si este índice es una respuesta correcta
            const isCorrect = data.respuestas_correctas &&
                (data.respuestas_correctas.includes(oIdx.toString()) ||
                    data.respuestas_correctas.includes(oIdx));
            renderOpcion(div.querySelector('.opciones-list'), pIdx, opc, isCorrect);
        });
    } else {
        renderOpcion(div.querySelector('.opciones-list'), pIdx);
    }
...
document.querySelectorAll('.delete-recurso-btn').forEach(button => {
    button.addEventListener('click', () => {

        const recursoId = button.dataset.id;
        const item = document.getElementById('recurso-item-' + recursoId);
        const input = document.getElementById('input-delete-recurso-' + recursoId);
...
document.addEventListener('click', function (e) {
    const button = e.target.closest('.remove-parent-img');
    if (!button) return;

    const card = button.closest('.imagen-card');
    const imagenId = button.dataset.id;

    if (!imagenId) {
        // Imagen nueva → eliminar definitivamente
        card.remove();
        return;
    }

    const inputDelete = document.getElementById('delete-imagen-' + imagenId);

    if (inputDelete.disabled) {
        // marcar para eliminar
